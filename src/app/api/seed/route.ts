...
export async function POST(request: NextRequest) {
  try {
    ...
    await Promise.all([
      ...
      prisma.userAction.create({
        data: {
          ...
          metadata: JSON.stringify({ challengeId: createdChallenges[0].id }),
          timestamp: today
        }
      })
    ])
    ...
    return NextResponse.json({
      message: 'Database seeded successfully with categories, challenges, users, prizes, advisors, email templates, submissions, and analytics data',
      stats: {
        ...
        analytics: 'Sample analytics data added'
      }
    })
